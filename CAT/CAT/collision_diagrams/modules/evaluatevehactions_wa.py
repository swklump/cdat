...
def dir_act(veh_num_dir_to,veh_num_action,file_length,crash_set,unknown_crashes,known_crashes,i):
    ...
    if veh_num_dir_to[i] in ['north','northeast','northwest','north-east','north-west']:
        if veh_num_action[i] == "making left turn":
            ...
            known_crashes.append(file_length[i])                   
        elif veh_num_action[i] in ['backing','slowing','starting from parked position',
                                   'starting in traffic lane',
                                   'stopped at signal or stop sign',
                                   'stopped in traffic','stopped in roadway']:
    ...
    elif veh_num_dir_to[i] in ['south','southeast','southwest','south-west',
                               'south-east']:
        if veh_num_action[i] == "making left turn":
    ...
    elif veh_num_dir_to[i] in ['vehicle stopped','vehicle backing']:
        crash_set["Crash"+str(i+1)].append('vehicle backing/stopped')
        known_crashes.append(file_length[i])               
