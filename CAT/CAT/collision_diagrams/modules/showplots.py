...
def show_plots(crash,fig_number,state,ped_bike_filter,diagram_total, diagram_fi):
    ...
    if 'pedestrian' in crash[0] or 'bicyclist' in crash[0]:
        ...
        for lookup, arrows in veh_dict.items():                                                                                                                                    
            if crash[0] == lookup:
                position = crash[0]
                draw_arrows_ped_bike(fig_number,arrows,crash,state)
    ...
    #In Scenario 2, at least one miscellaneous action
        ...
        for lookup, arrows in veh_dict.items():                                                                                                                                    
            if crash[0] == lookup:
                draw_arrows(fig_number,arrows,crash,state,ped_bike_filter)
    ...
    return fig_number, diagram_total, diagram_fi

#Allows for filtering of how many diagrams to show
def plot_loop(crashes_list,diagram_filter,state, ped_bike_filter, file_length):
    ...
    try:
        while fig_number <= diagram_filter:
            plots_temp = show_plots(crashes_list[i],fig_number,state, ped_bike_filter, diagram_total, diagram_fi)
    ...
    return total_accounted_for, fi_accounted_for